...
class ModsManager:
    ...
    def install_from_catalog(self, tag: str, version: str) -> InstalledMod:
        mods_dir = self.storage.paths.mods_dir
        target_dir = mods_dir / tag

        if target_dir.exists():
            self.remove(tag)

        zip_url = f"https://github.com/peshk0v/Zapret-Hub-Zen-Mods/archive/refs/tags/{tag}.zip"

        with tempfile.TemporaryDirectory(prefix="zapret_zen_catalog_") as temp_dir:
            zip_path = Path(temp_dir) / f"{tag}.zip"
            self.github.github_download(zip_url, zip_path, timeout=30, purpose="catalog-mod-download")
                ...
                for m in members:
                    if not m.is_dir():
                        ...
                        if len(parts) >= 2 and parts[1] == tag:
                            mod_prefix = f"{parts[0]}/{tag}/"
                            break

                if not mod_prefix:
                    raise ValueError(f"Мод '{tag}' не найден в архиве релиза.")

                for member in members:
                    if member.is_dir():
                        continue
                    if not member.filename.startswith(mod_prefix):
                        continue
                    rel = Path(member.filename[len(mod_prefix):])
                    if rel.is_absolute() or ".." in rel.parts:
                        continue
                    dest = extract_dir / rel
                    dest.parent.mkdir(parents=True, exist_ok=True)
                    with archive.open(member, "r") as src, dest.open("wb") as dst:
                        shutil.copyfileobj(src, dst)

            meta = {}
            meta_path = extract_dir / self.METADATA_FILENAME
            ...
            general_scripts = sorted(
                script.name for script in target_dir.glob("*.bat")
            ...
            entry = InstalledMod(
                ...
                author=str(meta.get("author", self.UNKNOWN_AUTHOR)),
                description=str(meta.get("description", "")),
                source_url=f"https://github.com/peshk0v/Zapret-Hub-Zen-Mods/releases/tag/{tag}",
                enabled=True,
                source_type="zapret_bundle",
                general_scripts=general_scripts,
                emoji=random.choice(self._EMOJI_CHOICES),
            ...
            self.logging.log("info", "Catalog mod installed", mod_id=tag, tag=tag)
            return entry
